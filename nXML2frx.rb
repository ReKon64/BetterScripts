...
OptionParser.new do |opts|
  ...
  opts.on('-xVALUE', '--extensions=VALUE', 'Space separated extensions put in " " to pass to ferox') do |value|
    options[:extensions] = value
  end

  opts.on('-wVALUE', '--wordlist=VALUE', 'Absolute path to wordlist to use. Defaults to combined.txt') do |value|
...
if options[:extensions].nil?
  puts "No extensions provided".red
else
  exe = "-x"
  exe += options[:extensions].to_s
end

if options[:wordlist].nil?
  puts "No wordlist provided. Defaulting to combined.txt".red
  options[:wordlist] = '/usr/share/wordlists/combined.txt'
end
  ...
  if details[:service_name] =~ /http/i  # Case-insensitive match for "http"
    pool.post do
      ...
      File.open(output_filename, 'w') do |file|
        command = "feroxbuster -u #{url}:#{port} --extract-links -x #{options[:extensions]} -B -C 404 -w=#{options[:wordlist]} -T 15 -t #{nThreads} -k --force-recursion --silent"
        #print command
        puts ("Busting #{url}:#{port} using #{options[:wordlist]} with #{nThreads} threads").light_cyan
